...
- **SC27 Special Introduction/Check-in Meeting — one-off for 2027, REMOVE after the season.** Three of the ten 2027 Summer Conferences run back-to-back with "Minneshögtid GML 100 år" (Gösta Mittag-Leffler's 100-year jubilee, itself excluded from `programs` as a memorial event — see below). Their organizers plus the jubilee's own contact get an extra Introduction/Check-in meeting 14 days before the ordinary shared one (`before(254)`/`before(104)` days, same Friday-snap placement), as a single meeting each — no Group 1/Group 2 split. Implemented as a small hardcoded block in [MeetingAgent.jsx](src/components/MeetingAgent.jsx) (constants `SC27_SPECIAL_DATE_RANGES_2027` / `SC27_SPECIAL_RULES`, generation block right before `generateMeetings`'s `programList.forEach`), matched by program **date range**, not name (titles drift between re-entry; dates don't) — deliberately **not** a `meetingRules['Summer Conference']` entry, since every rule there applies to all ten conferences and there's no way to scope one to just three named programs. This is a narrow, one-time exception to the "config not hardcode" rule above; a second occurrence in a future year should get the real named-subset-rule feature built instead of a second copy-pasted block.
